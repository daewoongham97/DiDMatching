...
beta_theta_post = 1.5; beta_x_post = 0
beta_theta_pre = c(1.25, 1.0); beta_x_pre = c(0, 0)
sig_x = sig_theta = 1
...
for (i in 1:length(rhos)) {
  ...
  b = beta_theta_pre[1]*beta_theta_pre[2] + beta_x_pre[1]*beta_x_pre[2] + 
    beta_theta_pre[1]*beta_x_pre[2]*rho + beta_theta_pre[2]*beta_x_pre[1]*rho
  Sigma_YY = matrix(c(a[1], b, b, a[2]), nrow = 2)
  
  # coefs = cbind(beta_theta_pre, beta_x_pre)
  # sigma_thetaX = matrix( c( sig_theta^2, rho,
  #                           rho, sig_x^2 ), nrow= 2 )
  # coefs %*% sigma_thetaX %*% t(coefs) + diag( rep( sigma_pre^2, 2 ) )
  ...
  rt = sig_theta_tilde*mean(beta_theta_pre^2)*2/(sigma_pre^2 + sig_theta_tilde*mean(beta_theta_pre^2)*2)
