...
plot_df = data.frame(time = rep(time, 3),
                     y = c(control_nomatch, control_match, trt_match),
                     group = rep(c("Same Principal (Unmatched)", "Same Principal (Matched)", "New Principal") ,
                                 each = 7))
library(ggplot2)
library(ggpubr)
library(cowplot)
