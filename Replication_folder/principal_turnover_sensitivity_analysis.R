...
r_theta = c(0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95)
...
for (i in 1:length(r_theta)) {
  res_stg = DiD_matching_guideline_staggered( Y_pre = pre_years,
                                              ...
                                              X = c_vars,
                                              data = dat,
                                              aggregate_only = TRUE, r_theta = r_theta[i])
  result[[i]] = res_stg
  match[i] = res_stg$statistic$statistic[1] >= (1 - abs(1 - res_stg$statistic$statistic[2]))
  #reduc_X[i] = res_stg$result$bias_reduction[1]
  reduc_Y[i] = res_stg$result$bias_reduction[2]
  slope[i] = as.numeric(res_stg$statistic[2, 2])
}
